...
class Concept_finder():
    ...
    def fit(self, corpus):
        #sentences = [s.split() for document in corpus for s in document]
        #sentences =[s for document in corpus for s in document]
        #print(len(sentences))
        #print(sentences[0:5])

        #sentences =[s for document in corpus for s in nltk.sent_tokenize(document)]
        #self.model = word2vec.Word2Vec([s.split() for s in sentences], size=100, window=5, min_count=5, workers=4)
        ...
        for kwl in keywords:
            for keyw in kwl:
                try:
                    ...
                    notfound += 1
                    pass
        print("found keys: {}, notfound keys: {}, total = {}".format(found, notfound,(found+notfound)))

        print(type(self.arrays))
        return self.arrays
...
class Clustering():
    ...
    def fit_transform(self, number_of_clusters):
        kmeans = KMeans(n_clusters=number_of_clusters, random_state=0).fit(self.w2vecarrays)
    ...
    def visualize(self):
        ...
        for row_id in range(0, len(self.w2v)):
            target_word = self.w2v[row_id][1]
            x = X_tsne[row_id, 0]
            y = X_tsne[row_id, 1]
            plt.annotate(target_word, (x,y))

        plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c=self.labels)
        plt.show()
